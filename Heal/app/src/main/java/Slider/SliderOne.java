...
public class SliderOne extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (getContext() != null) {
            textContent = getContext().getResources().getStringArray(R.array.motivational_quotes);
    ...
    private void updateRandomContent(TextView title, TextView description) {
        if (textContent != null && textContent.length > 0) {
            ...
            String quote = textContent[randomIndex];
            String[] parts = quote.split("–", 2);
            if (parts.length == 2) {
                title.setText(parts[0].trim());
                description.setText("–" + parts[1].trim());
            } else {
                title.setText("Motivational Quote");
                description.setText(quote);
            }
        }
    }
    ...
    private void addAndAnimateShapes(ConstraintLayout parentLayout, int numberOfShapes) {
        ...
        TextView descriptionTextView = parentLayout.findViewById(R.id.slideOneDescription);

...
public class SliderTwo extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (getContext() != null) {
            textContent = getContext().getResources().getStringArray(R.array.interesting_facts);
            shapeDrawables = getContext().getResources().obtainTypedArray(R.array.material_shapes);
        }

        TextView titleTextView = view.findViewById(R.id.slideTwoTitle);
        TextView descriptionTextView = view.findViewById(R.id.slideTwoDescription);
    ...
    private void updateRandomContent(TextView title, TextView description) {
        if (textContent != null && textContent.length > 0) {
            ...
            String fact = textContent[randomIndex];
            String[] parts = fact.split("–", 2);
            if (parts.length == 2) {
                title.setText(parts[0].trim());
                description.setText("–" + parts[1].trim());
            } else {
                title.setText("Interesting Fact");
                description.setText(fact);
            }
        }
    }
    ...
    private void addAndAnimateShapes(ConstraintLayout parentLayout, int numberOfShapes) {
        ...
        TextView descriptionTextView = parentLayout.findViewById(R.id.slideTwoDescription);
